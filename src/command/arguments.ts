...
export function parseAny(type: ArgumentTypes, val: string, message: Message): any {
    switch (type) {
        case 'string': return stringParser(val);
        case 'number': return numberParser(val);
        case 'member': return memberParser(val, message);
        case 'role': return roleParser(val, message);

        default: throw new Error('Trying to parse wrong type');
    }
}
