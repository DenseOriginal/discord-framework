...
async function parseAndValidateArgument(argument: Argument, input: string, message: Message) {
    ...
    if (isParsingError(parsedArgument)) {
        ...
        message.channel.send(
            createArgumentErrorEmbed(message, inputArgument, parsedArgument)
        );
    }
    ...
    if (validator) {
        ...
        if (Array.isArray(validator)) { validators.push(...validator); }
        ...
        for await (const validator of validators) {
            ...
            if (validatorReturn.status == 'error') {
                errorHappened = true;
                message.channel.send(
                    createArgumentErrorEmbed(message, inputArgument, { error: validatorReturn.message })
                );
                break;
            }
        }
