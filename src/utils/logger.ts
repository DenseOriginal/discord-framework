...
class Logger {
  ...
  disable(...labels: Labels[]) {
    labels.forEach((label) => this.disabled.set(label, true));
  }
  disableAll() {
    LabelsArray.forEach((l) => this.disable(l));
  }
  enable(...labels: Labels[]) {
    labels.forEach((label) => this.disabled.set(label, false));
  }
