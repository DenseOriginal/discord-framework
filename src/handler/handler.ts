    ...
    return function <T extends new (...args: any[]) => any>(target: T): T {
        class newTarget extends target implements HandlerInterface {
            ...
            async run(messageReader: MessageReader) {
                ...
                if(canRun instanceof FriendlyError && canRun.message) {
                    const canRunReturnErrorEmbed = createErrorEmbed(canRun, message.cleanContent);
                ...
                try {
                    ...
                    if((<any>commandOrHandler).execute) { (<CommandInterface>commandOrHandler).execute(messageReader) }
                    // Else if its a handler call run
                    else if((<any>commandOrHandler).run) { (<HandlerInterface>commandOrHandler).run(messageReader) }
                } catch (_err) {
            ...
            async canRun(messageToCheckAgainst: Message): Promise<FriendlyError | void> {
                ...
                try {
                    ...
                    const filteredResults: FriendlyError[] = canRunResults.filter(result => result instanceof FriendlyError) as FriendlyError[];
